...
class HybridRAG:
    def __init__(
        ...
        self.client = ollama.Client(
            host=os.getenv("OLLAMA_HOST", "http://ollama-service:11434")
        )
        self.embedder = SentenceTransformer(embedding_model)
        self.unique_contexts, self.dense_index, self.bm25_index = (
            self.build_index()
        )
    ...
    def build_index(self):
        ...
        context_embeddings = self.embedder.encode(
            unique_contexts, normalize_embeddings=True
        )
        dimension = context_embeddings.shape[1]
    ...
    def reciprocal_rank_fusion(
        self, dense_ranks: list[int], sparse_ranks: list[int]
    ) -> list[int]:
    ...
    def clean_completion(self, text: str) -> str:
        ...
        if (
            lowercased.startswith("unanswerable")
            or "not provided" in lowercased
            or "cannot be found" in lowercased
        ):
            return "Unanswerable"

        return text.rstrip(".")

    def run(self, question: str, top_k: int = None):
        k = top_k if top_k is not None else self.top_k

        # 1. Dense Retrieval
        q_emb = self.embedder.encode([question], normalize_embeddings=True)
        _, dense_indices = self.dense_index.search(
            np.array(q_emb, dtype=np.float32), self.top_k_dense
        )
        ...
        sparse_ranked_ids = np.argsort(sparse_scores)[::-1][
            : self.top_k_sparse
        ].tolist()

        # 3. Hybrid Fusion via RRF
        retrieved_ids = self.reciprocal_rank_fusion(
            dense_ranked_ids, sparse_ranked_ids
        )[:k]
